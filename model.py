...
class GAT(nn.Module):
    def __init__(self,num_layers,in_dim,num_hidden,heads,
    ...
    def forward(self, inputs, g):
        ...
        for l in range(self.num_layers):
            temp = h.flatten(1)
            h = self.gat_layers[l](g, temp, get_attention=False)
...
class CDGSN(nn.Module):
    def __init__(self,num_layers,in_dim,num_hidden,heads,
                 activation,feat_drop,attn_drop,negative_slope,residual,dropout,
                 input_dim_edge, input_dim_node, hid_dim_node_clf, hid_dim_edge_clf, Hid_dim_edge_domain_1,Hid_dim_edge_domain_2):
        super(CDGSN, self).__init__()
        self.network_embedding = GAT(num_layers,in_dim,num_hidden,heads,activation,feat_drop,attn_drop,negative_slope,residual)
        ...
        self.domain_discriminator = DomainDiscriminator(input_dim_edge, dropout, Hid_dim_edge_domain_1,Hid_dim_edge_domain_2)
        self.grl = GRL()

    def forward(self, features_s, features_t, g_s, g_t, edge_label_s_random, edge_label_t_random, edge_type):

        emb_node_s, heads_s = self.network_embedding(features_s, g_s)
        emb_node_t, heads_t= self.network_embedding(features_t, g_t)
        ...
        return pred_logit_s, pred_logit_t, pred_logit_node_s, pred_logit_node_t, d_logit, emb_node_s, emb_node_t